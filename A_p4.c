...
int main(void){
    ...
    for(int j=0; j<PROCESS_NUM; j++){
        ...
        if (-1 == pid[j]) {
        ...
        else if (0 == pid[j]) { // 子プロセス
            // 例．201枚のとき: プロセス1 : i=0~50, プロセス2 : i=51~101, プロセス3 : i=102~152, プロセス4 : i=153~200
            for(int i = 0 + (j*((img_file_count/PROCESS_NUM) + 1)); i < ((j+1)*((img_file_count/PROCESS_NUM) + 1)) && i < img_file_count; i++){
                JpegData jpegData;
            ...
            #ifdef _DEBUG
            if(j==0) printf("p0_count : %d\n", p0_count);
            if(j==1) printf("p1_count : %d\n", p1_count);
            if(j==2) printf("p2_count : %d\n", p2_count);
            if(j==3) printf("p3_count : %d\n", p3_count);
            #endif

            gettimeofday(&t1, NULL);
            timersub(&t1, &t0, &t1);
            printf("child[%d]: %ld.%06ld\n", j, t1.tv_sec, t1.tv_usec);
            exit(EXIT_SUCCESS);
        }
    }

    int status;
    for(int i=0; i<PROCESS_NUM; i++) wait(&status); // 子プロセスの個数分waitする
