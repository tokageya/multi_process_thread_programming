...
#define MAX_PATH_LEN 1000 // ファイルの絶対パスの長さ上限
#define MAX_FILE_NUM 1000 // 読み込むjpgファイルの数上限
...
int main(void){
    DIR *dir;
    struct dirent *dp;
    char dirpath[MAX_PATH_LEN] = "/home/msait/Downloads/all_jpg/"; // 大量のjpgファイルがあるディレクトリ
    char input_jpg_file_name_list[MAX_FILE_NUM][MAX_PATH_LEN]; // 大量の入力用jpgファイル名(絶対パス)
    char output_jpg_file_name_list[MAX_FILE_NUM][MAX_PATH_LEN]; // 大量の出力用jpgファイル名(絶対パス)

    /* 入力・出力用のjpgファイル名(絶対パス)のリストを作成 */
    dir = opendir(dirpath);
    ...
    while (dp != NULL) {
        if( strcmp(dp->d_name, "..") && strcmp(dp->d_name, ".") ){
            ...
            char dst[MAX_PATH_LEN] = "/home/msait/Downloads/all_output_jpg/"; // 色反転したjpgファイルの出力先ディレクトリ
